      ...
      <section className='key-features'>
        <Container maxWidth="lg">
          <Grid container spacing={3}>
            ...
            {[{ icon: <LibraryMusicIcon />, title: 'Pitch Control', desc: 'Shift audio up or down semitones.' },
              { icon: <SpeedIcon />, title: 'Speed Control', desc: 'Speed up or slow down music and voice.' },
              { icon: <VolumeUpIcon />, title: 'Fade In/Out', desc: 'Smooth transitions for tracks.' },
              { icon: <HighQualityIcon />, title: 'Normalize', desc: 'Auto-adjust volume for consistency.' },
              { icon: <VolumeUpIcon />, title: 'Volume Boost', desc: 'Increase or decrease audio levels.' },
              { icon: <DownloadIcon />, title: 'Instant Download', desc: 'Export effects-applied audio quickly.' }].map((feat, i) => (
                <Grid key={i} size={{ xs: 12, sm: 6, lg: 4 }}>
                  <Card sx={{ height: '100%' }}>
                    <CardContent sx={{ p: 2 }}>
                      {feat.icon}
