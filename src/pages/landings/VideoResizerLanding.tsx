      ...
      <section className='key-features'>
        <Container maxWidth="lg">
          <Grid container spacing={3}>
            ...
            {[{ icon: <UploadFileIcon />, title: 'Drag & Drop Upload', desc: 'Quick video importing.' },
              { icon: <PreviewIcon />, title: 'Live Preview', desc: 'See resolution and ratio before exporting.' },
              { icon: <SettingsIcon />, title: 'Aspect Ratio Presets', desc: '16:9, 9:16, 4:3, 1:1, 21:9.' },
              { icon: <SpeedIcon />, title: 'FPS Control', desc: 'Adjust frame rate for smoother playback.' },
              { icon: <VideoFileIcon />, title: 'Cross-Browser', desc: 'Works on Chrome, Safari, Firefox, Edge.' },
              { icon: <LanguageIcon />, title: 'Instant Download', desc: 'Export resized MP4 in one click.' }].map((feat, i) => (
                <Grid key={i} size={{ xs: 12, sm: 6, lg: 4 }}>
                  <Card sx={{ height: '100%' }}>
                    <CardContent sx={{ p: 2 }}>
                      {feat.icon}
